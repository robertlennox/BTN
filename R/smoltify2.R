...
smoltify2<-function (meta, receivers, detections)
{
  ...
  m <- meta %>% dplyr::mutate(ID = as.numeric(.data$ID)) %>%
    dplyr::distinct(.data$Vendor, .data$tagCodeType, .data$ID,
                    .data$n_ID, .data$redeploy) %>% dplyr::mutate(n_ID = n_ID -
                                                                    1) %>% dplyr::mutate(ID2 = dplyr::case_when(.data$n_ID ==
                                                                                                                  0 ~ NA_real_, .data$n_ID > 0 ~ ID + 1)) %>% dplyr::mutate(ID3 = dplyr::case_when(.data$n_ID >=
                                                                                                                                                                                                     2 ~ .data$ID + 2, T ~ NA_real_)) %>% dplyr::mutate(ID4 = dplyr::case_when(.data$n_ID ==
                                                                                                                                                                                                                                                                                 3 ~ .data$ID + 3, T ~ NA_real_)) %>% dplyr::mutate(ID3 = dplyr::case_when(.data$n_ID >=
                                                                                                                                                                                                                                                                                                                                                             2 ~ .data$ID + 2, T ~ NA_real_)) %>% dplyr::mutate(oid = .data$ID) %>%
    dplyr::select(-.data$n_ID) %>% tidyr::gather(key, value,
                                                 -oid, -Vendor) %>% dplyr::rename(ID = oid) %>% right_join(meta %>%
                                                                                                             mutate(ID = as.numeric(.data$ID))) %>% dplyr::mutate(key = case_when(grepl("-AT",
                                                                                                                                                                                        .data$Transmitter) & value - ID == 0 ~ "temp", grepl("A-LP",
                                                                                                                                                                                                                                             .data$Transmitter) ~ "accel", grepl("-AT", .data$Transmitter) &
                                                                                                                                                                                    ...
                                                                                                                                                                                    value - ID == 1 ~ "eaten", grepl("-TAD", .data$Transmitter) &
                                                                                                                                                                                    value - ID == 0 ~ "temp", grepl("-TAD", .data$Transmitter) &
                                                                                                                                                                                    value - ID == 1 ~ "accel", grepl("-TAD", .data$Transmitter) &
    ...
    mutate(fatedate = lubridate::parse_date_time(.data$fatedate,
                                                 c("dmy", "dmy_HM"))) %>% dplyr::rename(Project = "\nProject")
  ...
  dets <- detections %>% dplyr::mutate(dti = lubridate::date(.data$dt)) %>%
    left_join(receiver_locations, by = c("serial", "dti")) %>%
    dplyr::left_join(m %>% dplyr::filter(is.na(redeploy)) %>%
                       dplyr::mutate(ID = as.integer(ID)) %>% dplyr::rename(Project = 5) %>%
                       dplyr::select(ID, oid, tagCodeType, dmy, sensor,
                                     Spp, TL, Angler, fate, fatedate, Sex, Project,
                                     Transmitter, "Capture site", "Release Site"),
                     by = c("ID", "tagCodeType")) %>% dplyr::filter(lubridate::date(.data$dt) >=
                                                                      .data$dmy) %>%
    bind_rows(detections %>% dplyr::filter(ID %in% redeploys$ID) %>%
                left_join(redeploys) %>% dplyr::filter(lubridate::date(.data$dt) >=
                                                         dmy) %>% dplyr::select(-.data$dmy) %>% dplyr::mutate(dti = lubridate::date(.data$dt)) %>%
                left_join(receiver_locations, by = c("serial", "dti")) %>%
                dplyr::left_join(m %>% dplyr::filter(.data$redeploy ==
                                                       T) %>% dplyr::mutate(ID = as.integer(ID)) %>%
                                   dplyr::rename(Project = 5) %>% dplyr::select(ID,
                                                                                oid, tagCodeType, dmy, sensor, Spp, TL, Angler,
                                                                                fate, fatedate, Sex, Project, Transmitter, "Capture site",
